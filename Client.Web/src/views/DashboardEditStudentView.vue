...
import { mapDetalleToEstudiante } from '@/helpers/mapDetalleToEstudiante';
...
onMounted(async () => {
  ...
  studentId.value = parsedId;
  await cargarDatos(parsedId);
...
async function cargarDatos(id: number) {
  try {
    const response = await studentService.getInscripcionCompleta(id);
    if (response.success && response.payload) {
      student.value = mapDetalleToEstudiante(response.payload);
    } else {
      sections.value = [];
    }
  } catch (error) {
...
async function updateStudent() {
  ...
  try {
    const response = await studentService.update(studentId.value!, student.value);

    if (response.success) {
      message.value = 'Estudiante actualizado correctamente';
      ToastService.success('Se ha actualizado el estudiante exitosamente');
    ...
    ToastService.error('Error al actualizar al estudiante, revise la informacion suministrada');
